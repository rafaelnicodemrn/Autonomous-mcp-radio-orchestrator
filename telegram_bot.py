...
from content_enricher import enrich_item, deduplicate, score_item
from telegram_sender import (
    send_item_card, send_audio, send_text,
    send_briefing_header, send_section_header,
)
...
from adaptive_engine import (
    load_state as load_adaptive_state, save_state as save_adaptive_state,
    record_feedback, record_command_usage, update_source_reputation,
    compute_adaptive_score, sync_youtube_signals, run_weekly_analysis,
    format_learning_status, calculate_dynamic_weights,
)
...
BRIEFING_SOURCES = [
    'biblia', 'utilidades', 'catolicismo',
    'noticias', 'tecnologia', 'politica', 'conservadorismo',
]
    ...
    for i, (t, name, dur, folder, has_mp3) in enumerate(episodes):
        ...
        if has_mp3:
            keyboard.append([InlineKeyboardButton(
                f'🎵 {name}', callback_data=f'play:{folder}'
            )])
...
async def callback_feedback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass
    await query.message.reply_text(f'{emoji} Obrigado pelo feedback! Isso ajuda a melhorar suas recomendações.')
    ...
    if cmd == 'add':
        ...
        if tipo == 'interesse':
            if valor not in profile['interesses_primarios']:
                ...
                await update.message.reply_text(
                    f'✅ Adicionado aos seus interesses: <b>{valor}</b>',
                    parse_mode='HTML'
                )
            else:
                await update.message.reply_text(
                    f'ℹ️ Você já tem esse interesse: <b>{valor}</b>',
                    parse_mode='HTML'
                )
            ...
            if valor not in profile['ignorar_sempre']:
                ...
                save_profile(profile)
                await update.message.reply_text(
                    f'✅ Adicionado à sua lista de ignorar: <b>{valor}</b>',
                    parse_mode='HTML'
                )
            else:
                await update.message.reply_text(
                    f'ℹ️ Você já ignora isso: <b>{valor}</b>',
                    parse_mode='HTML'
                )
            ...
            if valor not in profile['fontes_vip']:
                ...
                save_profile(profile)
                await update.message.reply_text(
                    f'⭐ Adicionado às suas fontes VIP: <b>{valor}</b>',
                    parse_mode='HTML'
                )
            else:
                await update.message.reply_text(
                    f'ℹ️ Essa fonte já é VIP: <b>{valor}</b>',
                    parse_mode='HTML'
                )
        ...
        if tipo == 'interesse':
            ...
            if encontrado:
                ...
                save_profile(profile)
                await update.message.reply_text(
                    f'✅ Removido dos seus interesses: <b>{encontrado}</b>',
                    parse_mode='HTML'
                )
            else:
                await update.message.reply_text(
                    f'❌ Interesse não encontrado: {valor}',
                    parse_mode='HTML'
                )
            ...
            if encontrado:
                ...
                save_profile(profile)
                await update.message.reply_text(
                    f'✅ Removido da sua lista de ignorar: <b>{encontrado}</b>',
                    parse_mode='HTML'
                )
            else:
                await update.message.reply_text(
                    f'❌ Palavra-chave não encontrada: {valor}',
                    parse_mode='HTML'
                )
            ...
            if encontrado:
                ...
                save_profile(profile)
                await update.message.reply_text(
                    f'✅ Removido das suas fontes VIP: <b>{encontrado}</b>',
                    parse_mode='HTML'
                )
            else:
                await update.message.reply_text(
                    f'❌ Fonte VIP não encontrada: {valor}',
                    parse_mode='HTML'
                )
        ...
        if tipo == 'score':
            ...
            if 1 <= valor <= 20:
                ...
                save_profile(profile)
                await update.message.reply_text(
                    f'✅ Máx. cards por comando alterado para <b>{valor}</b>',
                    parse_mode='HTML'
                )
            else:
                await update.message.reply_text(
                    '❌ Número de cards deve estar entre 1 e 20.',
                    parse_mode='HTML'
                )
...
async def cmd_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    url = args[0]
    await update.message.reply_text(f'⏳ Gerando episódio a partir de:\n{url}\n\nAguarde alguns minutos...')
    ...
    if not items:
        await update.message.reply_text('⚠️ Nenhum conteúdo gerado. Verifique se a URL é acessível.')
        return
...
async def cmd_generate(update: Update, context: ContextTypes.DEFAULT_TYPE,
                       sources: list, label: str, cmd_key: str = ''):
...
def main():
    ...
    for cmd, sources in COMMANDS.items():
        ...
        app.add_handler(CommandHandler(cmd.replace('/', ''),
                        _make_command_handler(sources, label, cmd)))
