...
def score_batch_with_gemini(items: list, profile: dict) -> list:
    ...
    try:
        ...
        if match:
            ...
            score_map = {s["i"]: (int(s.get("score", 5)), str(s.get("motivo", ""))) for s in scores}
            for i, item in enumerate(items):
...
def filter_and_score_items(items: list, profile: dict) -> list:
    ...
    """
    from content_enricher import _is_english, diversity_guard, translate_if_needed
...
def format_profile(profile: dict) -> str:
    ...
    lines = [
        f"👤 <b>Perfil de {nome} — RadioIA Pessoal</b>\n",
