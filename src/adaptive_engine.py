...
DEFAULT_STATE = {
    ...
    'source_reputation': {},
    # Ex: {"Vatican News": {"total_score": 248, "count": 31, "avg": 8.0, "last_updated": "2026-06-13"}}
...
def update_source_reputation(source_name: str, score: int, state: dict = None) -> dict:
    ...
    entry = reputation.get(source_name, {
        'total_score': 0, 'count': 0, 'avg': 5.0, 'last_updated': None,
    })
...
def record_feedback(item_hash: str, source_name: str, source_id: str,
                    gemini_score: int, feedback: str):
...
def calculate_dynamic_weights(state: dict) -> dict:
    ...
    if tem_feedback and tem_reputacao and tem_youtube:
        return {'llm': 0.35, 'reputation': 0.20, 'recency': 0.15,
                'feedback': 0.20, 'youtube': 0.10}
    elif tem_feedback and tem_reputacao:
        return {'llm': 0.40, 'reputation': 0.25, 'recency': 0.20,
                'feedback': 0.15, 'youtube': 0.00}
    elif tem_reputacao and tem_youtube:
        return {'llm': 0.45, 'reputation': 0.25, 'recency': 0.20,
                'feedback': 0.00, 'youtube': 0.10}
    elif tem_reputacao:
        return {'llm': 0.55, 'reputation': 0.25, 'recency': 0.20,
                'feedback': 0.00, 'youtube': 0.00}
    else:
        # Estado inicial: só LLM e recência
        return {'llm': 0.70, 'reputation': 0.00, 'recency': 0.30,
                'feedback': 0.00, 'youtube': 0.00}
...
def _calc_youtube_alignment(item: dict, state: dict) -> float:
    ...
    matched_weights = [
        weight for interest, weight in vector.items()
...
def sync_youtube_signals(youtube_service) -> dict:
    ...
    try:
        ...
        channel_resp = youtube_service.channels().list(
            part='contentDetails',
            mine=True,
        ).execute()
        ...
        if liked_playlist_id:
            items_resp = youtube_service.playlistItems().list(
                part='snippet',
    ...
    try:
        ...
        if watch_history_id:
            hist_resp = youtube_service.playlistItems().list(
                part='snippet',
